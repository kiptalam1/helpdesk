...
function RegisterForm() {
  ...
  const [errors, setErrors] = useState<ZodInputErrors>({
    username: [],
    email: [],
    password: [],
    confirmPassword: [],
  ...
  async function handleRegisterUser(e: SyntheticEvent) {
    e.preventDefault();
    try {
      setLoading(true);
      ...
      if (!parsed.success) {
        ...
        console.warn("zoderrors: ", fieldErrors, errors);
  ...
  return (
    <div className="border border-border shadow-sm bg-card px-5 py-8 rounded-lg">
      <form onSubmit={handleRegisterUser} className="flex flex-col gap-4 w-full">
        {/* username */}
